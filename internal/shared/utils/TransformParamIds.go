...
func TransformParamIDS(params url.Values, dest map[string]any) map[string]any {
	...
	for key := range params {
		...
		if err == nil {
			...
			fmt.Println("parsed key,valye is", key, queryValues[key])
			continue
		}
		...
		if parsedTime, err := time.Parse(time.RFC3339, value); err == nil {
			fmt.Println("parsed key,valye is", key, value)
			//delete the old value because we will replace it with new objectid
	...
	if dest != nil {
		for k, v := range queryValues {
			...
			if _, isObjectID := v.(primitive.ObjectID); isObjectID {
				//for object ids we dont need to regex them objectIDFromhex above will auto reject invalid objectids
				dest[k] = value
				continue
			}
